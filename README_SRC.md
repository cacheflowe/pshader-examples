...
<!-- @import examples/example_01_solid_color/example_01_solid_color.pde lang:java -->

**shader.glsl**

<!-- @import examples/example_01_solid_color/data/shader.glsl lang:glsl -->
...
## When Things Go Wrong: Debugging Shaders

Things will go wrong - broken shaders are a normal part of the process. Unlike Processing code, where errors often point to a specific line number with a helpful message, shader errors can be cryptic or produce no visible output at all. A few practical strategies can save time and frustration.

**Check the console for compilation errors.** When a shader fails to compile, Processing prints the GLSL compiler's error message in the console. These messages identify the line number and the type of error, though the wording can be terse. A message like `ERROR: 0:5: 'main' : syntax error` means line 5 of the shader has a syntax problem. If the console shows no shader errors but the output still isn't right, the shader is compiling and running - it's just not doing what's expected.

**If the canvas is black (or wrong), output a solid color first.** Replace the `gl_FragColor` line with a known value like `gl_FragColor = vec4(1.0, 0.0, 1.0, 1.0);` (bright magenta). If the solid color appears, the shader is loaded and running correctly, and the problem is in the logic. If the color doesn't appear, the issue is likely on the Processing side - check that `P2D` or `P3D` is set in `size()`, that `loadShader()` is pointing to the correct file, and that `filter()` or `shader()` is being called.

**Misspelled uniform names fail silently.** If a uniform is declared as `uniform float uTime;` in the shader but set as `myShader.set("time", value)` in Processing (without the `u` prefix, or with different capitalization), no error is thrown - the uniform simply stays at its default value of `0`. If a uniform doesn't seem to have any effect, double-check that the name in `set()` matches the declaration in the shader exactly.

**Visualize values as colors.** When a uniform or calculation isn't behaving as expected, output it directly as a color to see what's happening. For example, to verify that a `float` uniform is being received, set `gl_FragColor = vec4(myValue, 0.0, 0.0, 1.0);` - if the canvas turns red, the value is arriving. This technique of using color to inspect data is one of the most useful debugging tools in shader development, and will come up again throughout this tutorial.
...
_Normalized coordinate system diagram_
...
<!-- @import examples/example_02_uv_gradient/data/shader.glsl lang:glsl -->
...
* The output draws a black-to-red gradient, as the x coordinate is fed into the red color component of the `gl_FragColor` assignment. This demonstrates normalized coordinate values working in tandem with normalized color values.
...
* In shader programming, a pixel's location (relative to the triangle that it's drawn onto) is called its [UV coordinate](https://en.wikipedia.org/wiki/UV_mapping), which has roots in 3D modeling. Even in seemingly 2-dimensional shader programs, the GPU always needs geometry to shade. When `filter()` is called, Processing draws a full-canvas rectangle behind the scenes - two triangles that cover the entire canvas. The shader then runs on every pixel within those triangles. This underlying mesh is invisible to the developer, but it's how the GPU operates and why UV coordinates exist even in flat 2D shaders.
...
<!-- @import examples/example_03_uv_map/data/shader.glsl lang:glsl -->
...
<!-- @import examples/example_04_uniforms_mouse/example_04_uniforms_mouse.pde lang:java -->

In the shader code, the `uSplitX` value is accessed as a uniform:

**shader.glsl**

<!-- @import examples/example_04_uniforms_mouse/data/shader.glsl lang:glsl -->
...
<!-- @import examples/example_05_texture_grayscale/example_05_texture_grayscale.pde lang:java -->

In the shader program, the existing pixels drawn to the canvas by `image()` can be accessed by calling `filter()`.

**shader.glsl**

<!-- @import examples/example_05_texture_grayscale/data/shader.glsl lang:glsl -->
...
Another powerful feature of GLSL is **[swizzling](https://www.khronos.org/opengl/wiki/Data_Type_(GLSL)#Swizzling)**, which allows multiple components of a vector to be accessed in different orders or combinations. For instance, requesting a `vec2` from a `vec4` by calling `vertTexCoord.xy` is a form of swizzling.
...
**sketch.pde**
<!-- @import examples/example_06_cpu_pixels/example_06_cpu_pixels.pde lang:java -->
...
**sketch.pde**
<!-- @import examples/example_07_texture_filter/example_07_texture_filter.pde lang:java -->

**shader.glsl**
<!-- @import examples/example_07_texture_filter/data/shader.glsl lang:glsl -->
...
<!-- @import examples/example_08_sdf_circle/example_08_sdf_circle.pde lang:java -->

**shader.glsl**

<!-- @import examples/example_08_sdf_circle/data/shader.glsl lang:glsl -->
...
<!-- @import examples/example_21_noise_grain/example_21_noise_grain.pde lang:java -->

**shader.glsl**

<!-- @import examples/example_21_noise_grain/data/shader.glsl lang:glsl -->
...
**sketch.pde**
<!-- @import examples/example_13_texture_wrap/example_13_texture_wrap.pde lang:java -->

**uv-adjust.glsl**
<!-- @import examples/example_13_texture_wrap/data/uv-adjust.glsl lang:glsl -->
...
<!-- @import examples/example_22_kernel_blur/example_22_kernel_blur.pde lang:java -->

**shader.glsl**

<!-- @import examples/example_22_kernel_blur/data/shader.glsl lang:glsl -->
...
**sketch.pde**
<!-- @import examples/example_17_vertex_shader/example_17_vertex_shader.pde lang:java -->

**vert.glsl**
<!-- @import examples/example_17_vertex_shader/data/vert.glsl lang:glsl -->

**frag.glsl**
<!-- @import examples/example_17_vertex_shader/data/frag.glsl lang:glsl -->
...
- Vertex positions exist in their "screen space" coordinate system. For example, `vertex(200, 200)` creates a vertex that lives at 200, 200 in the vertex shader code too. This is a bit different than our normalized coordinates in the fragment shader.  
...
- Finally, there are some new concepts to learn about how data is passed between Processing and GLSL, and between the vertex and fragment shaders themselves. In the vertex shader, there are several new variables that are automatically provided by Processing to help understand the geometry being drawn.
  ...
  - Both `color` and UV coordinates (`texCoord`) are passed to the fragment shader via `varying` variables. A `varying` variable is used to pass data from the vertex shader to the fragment shader. There is always a vertex shader that runs before the fragment shader. Processing provides a [default vertex shader](https://github.com/processing/processing4/tree/main/core/src/processing/opengl/shaders) if one is not provided.
...
**sketch.pde**
<!-- @import examples/example_18_vertex_colors/example_18_vertex_colors.pde lang:java -->

**vert.glsl**
<!-- @import examples/example_18_vertex_colors/data/vert.glsl lang:glsl -->

**frag.glsl**
<!-- @import examples/example_18_vertex_colors/data/frag.glsl lang:glsl -->
...
**sketch.pde**
<!-- @import examples/example_19_vertex_generative/example_19_vertex_generative.pde lang:java -->

**vert.glsl**
<!-- @import examples/example_19_vertex_generative/data/vert.glsl lang:glsl -->

**frag.glsl**
<!-- @import examples/example_19_vertex_generative/data/frag.glsl lang:glsl -->
...
**sketch.pde**
<!-- @import examples/example_20_vertex_displacement/example_20_vertex_displacement.pde lang:java -->

**vert.glsl**
<!-- @import examples/example_20_vertex_displacement/data/vert.glsl lang:glsl -->

**frag.glsl**
<!-- @import examples/example_20_vertex_displacement/data/frag.glsl lang:glsl -->
...
<!-- @import examples/example_23_point_shader/example_23_point_shader.pde lang:java -->

**vert.glsl**

<!-- @import examples/example_23_point_shader/data/vert.glsl lang:glsl -->

**frag.glsl**

<!-- @import examples/example_23_point_shader/data/frag.glsl lang:glsl -->
...
- Using textures as data (heightmaps, etc.)
- Multiple render passes
- Using FBOs with shaders
- Using OpenGL features directly
- Using compute shaders (if supported)
