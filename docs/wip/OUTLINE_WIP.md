...
- ✅ Explain: varying values - passed from vertex shader to fragment shader  
  - ✅ Terrain example between CPU -> GPU (example_19_vertex_generative)
- ✅ Explain: attributes  
  - Custom attributes would be cool - are these only on PShape? And not in the official documentation despite being public method `attrib()` → TODO #4.3
- ✅ Advanced tutorial content (Based on Andres' Android tutorial) → Phase 4 ongoing
- Gradient on a circle - run a shader on a PGraphics and apply as a texture to a circle → TODO #3.5
...
- Landscape built in example shows how to use shadertoy code? _(unclear/untracked)_
- PShape! → TODO #4.3
...
* Extra Vertex shader notes - TODO: merge these into the outline above
  ...
  * ✅ Line shader, point shader types, texlight → TODO #4.5 (line example), #4.2 (lighting)
    * Fill + stroke gets more complicated → TODO #4.5
  ...
  * Particles - check points demo in 50 shades dir → TODO #4.4
  * Lighting basics (normals, light position, etc) → TODO #4.2
  - custom lighting:
...
- Add to post-processing section?
  - Compositing with multiple images - using a matte or mask → TODO #3.3
    - Example: use mouseX to blend between two images → TODO #3.3
    - Explain: how to pass multiple textures into a shader as uniforms → TODO #3.3
...
- ✅ Come back to drawing generative shapes with fragment shaders now that we've covered coordinates, uniforms, textures
  ...
  - ✅ MATH - use circle example
    ...
    - map() function doesn't exist. show custom implementation → TODO #2.9
      ```glsl
...
- PGraphics can use shaders for more advanced multi-canvas rendering & compositing → TODO #3.5
...
- PShape advanced ideas → TODO #4.3
  - `attrib()` custom attributes → TODO #4.3
...
- Simulation ideas?  
  ...
  - Cellular automata? https://github.com/processing/processing-examples/tree/main/Topics/Shaders
    - `ppixels`!!!!! Conway example - simulation shaders → TODO #6.1
